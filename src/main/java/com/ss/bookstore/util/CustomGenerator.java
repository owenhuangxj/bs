...
public class CustomGenerator {
	public static void main(String[] args) throws InterruptedException {
		 ...
		 */
		globalConfig.setIdType(IdType.ID_WORKER_STR);//idWorker 的字符串表示->分布式高效ID生产黑科技(sequence)只有当插入对象ID 为空，才自动填充
		...
		dataSourceConfig.setUrl("jdbc:mysql://localhost:3306/test?useUnicode=true&characterEncoding=UTF-8&generateSimpleParameterMetadata=true");
		...
		strategy.setTablePrefix(new String[] { "t_", "tsys_" });// 此处可以修改为你的表前缀
		...
		packageConfig.setParent("com.ss");//配置父包名,如果为空，将下面子包名必须写全部， 否则就只需写子包名
		packageConfig.setModuleName("bookstore");//配置父包模块名
